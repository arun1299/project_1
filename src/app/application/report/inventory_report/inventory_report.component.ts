...
import { invoice_details, sales_report } from './../report-models';
...
import { inventory_models } from './inventory_report.models';
...
export class Inventory_reportComponent implements OnInit {
  ...
  date!: FormGroup;
   ...
   dataSource   !: MatTableDataSource<inventory_models>;
   public totalPages      = 0;
   public Toggledata      = false;
   public      show       = false;
   public searchDataValue =''
    list          :Array<inventory_models> =[];
    from_date     : any
    to_date       : any
  ...
  async get_tabledata()
  {
    ...
    this.list   = [];
   await  this.api.post('mp_inventory_summary_report.php?from_date=' + this.from_date + '&to_date=' + this.to_date + '&authToken=' + environment.authToken, null).then((data: any[]) => {

           let value : inventory_models [] = []
          if (data != null)
           {

            let total_data = (data as any)['report'];
            this.totalData    = total_data.length
            total_data.map((res: inventory_models, index: number) => {
           ...
           this.calculateTotalPages(this.totalData, this.pageSize);
            this.dataSource = new MatTableDataSource<inventory_models>(value);
     }
  ...
  {
    this.list = [];
    await this.dataSource.filteredData.map((res: inventory_models, index: number) =>
      {
        const serialNumber = index + 1;
  ...
  exportExcel()
  {
          const exportData: Partial<TableElement>[] =
                this.dataSource.filteredData.map((x) => ({
                    ...
                    "Batch Number "     : x.Batch_Number,
                    "Quantity Ordered"  : x.Quantity_Ordered,
                    "Quantity In"       : x.Quantity_In,
                    "Quantity Out"      : x.Quantity_Out,
                    "Stock on Hand"     : x.Stock_on_Hand

                }));
          TableExportUtil.exportToExcel(exportData, 'Inventory summary Report');
  }

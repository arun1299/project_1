...
import { Employee_account_models } from './employee_account_report.models';
import { calendar } from 'ngx-bootstrap/chronos/moment/calendar';

@Component({
  selector: 'app-employee_account_report',
  templateUrl: './employee_account_report.component.html',
  styleUrls: ['./employee_account_report.component.scss']
})
export class Employee_account_reportComponent implements OnInit {
  ...
  dataSource   !: MatTableDataSource<Employee_account_models>;
  public totalPages = 0;
  public Toggledata  = false;
  public      show   = false;
  public searchDataValue =''
   list          :Array<Employee_account_models> =[];
   from_date       : any;
   to_date         : any;
   employee_list   : any;
   exp_id          : any;
 ...
 async get_tabledata()
 {
   ...
   this.api.get('mp_emp_account_report.php?value='+this.exp_id+'&from_date=' + this.from_date + '&to_date=' + this.to_date + '&authToken=' + environment.authToken).then((data: any) => {

    if (data != null)
      {
            let total_data = (data as any)['report'];
            let value :  Employee_account_models []= []
           this.totalData    = total_data.length
           total_data.map((res: Employee_account_models, index: number) => {
       ...
       this.dataSource = new MatTableDataSource<Employee_account_models>(value);
    }
 ...
 {
   this.list = [];
   await this.dataSource.filteredData.map((res: Employee_account_models, index: number) =>
     {
       const serialNumber = index + 1;
 ...
 exportExcel()
 {
   ...
   this.dataSource.filteredData.map((x) => ({
       "#"               : x.s_no,
       "Date"            : x.tran_date,
       "Mode"            : x.tran_mode === 1 ? "Wire" : (x.tran_mode === 2 ? "Case":"Cheque"),

...
import { payment_received } from './payment_received.models';

@Component({
  selector: 'app-payment_received_report',
  templateUrl: './payment_received_report.component.html',
  styleUrls: ['./payment_received_report.component.scss']
})
export class Payment_received_reportComponent implements OnInit {
  ...
  date!: FormGroup;
   ...
   dataSource   !: MatTableDataSource<payment_received>;
   public totalPages  = 0;
   public Toggledata  = false;
   public      show   = false;
   public searchDataValue =''
    list          : Array<payment_received> =[];
    from_date     : any
    to_date       : any
  ...
  async get_tabledata()
  {
    ...
    this.api.post('mp_payment_received_report.php?from_date=' + this.from_date + '&to_date=' + this.to_date + '&authToken=' + environment.authToken, null).then((data: any[]) => {
      ...
      if (data != null)
        {
            ...
            let value : payment_received [] = []
            this.totalData    = total_data.length
            total_data.map((res: payment_received, index: number) => {
         ...
         this.dataSource = new MatTableDataSource<payment_received>(value);
     }
     else{
  ...
  {
    this.list = [];
    await this.dataSource.filteredData.map((res: payment_received, index: number) =>
      {
        const serialNumber = index + 1;
  ...
  exportExcel()
  {
    ...
    this.dataSource.filteredData.map((x) => ({
        ...
        "Invoice Number"   : x.Invoive_No,
        "Deposited To"     : x.Deposited_To,
        "Amount"           : x.Amount,
        "Reference"        : x.reference,
        "Notes"            : x.notes
    }));
    TableExportUtil.exportToExcel(exportData, 'Payment Received Report');
   }

...
import { Epf_report } from './epf_report.models';

@Component({
  selector: 'app-epf_report',
  templateUrl: './epf_report.component.html',
  styleUrls: ['./epf_report.component.scss']
})
export class Epf_reportComponent implements OnInit {
  ...
  dataSource   !: MatTableDataSource<Epf_report>;
  public totalPages = 0;
  public Toggledata  = false;
  public      show   = false;
  public searchDataValue =''
   list        :Array<Epf_report> =[];
   date        !: FormGroup;
   from_date   : any
   to_date     : any
   bill_id     : any
 ...
 constructor(public fb: FormBuilder,public api :ApiService,public toastService : ToasterService)
 {
  ...
  async get_tabledata()
  {
    ...
    this.list   = [];
    this.api.post('mp_epf_report.php?from_date=' + this.from_date + '&to_date=' + this.to_date + '&authToken=' + environment.authToken, null).then((data: any[]) => {
      if (data != null)
        {
            ...
            let value : Epf_report []= []
            this.totalData    = total_data.length
            total_data.map((res: Epf_report, index: number) => {
       ...
       this.dataSource = new MatTableDataSource<Epf_report>(value);
     }
     else{
  ...
  async callData()
    {
      this.list = [];
      await this.dataSource.filteredData.map((res: Epf_report, index: number) =>
        {
          const serialNumber = index + 1;
  ...
  exportExcel()
  {
    ...
    TableExportUtil.exportToExcel(exportData, 'EPF Report');
  }

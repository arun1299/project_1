...
import { Customer } from './vendor-models';
...
export class VendorComponent  {
 ...
 async getTableData()
  ...
  this.pageNumberArray       = [];
   ...
   await this.api.get('mp_vendor_info.php?&authToken=' + environment.authToken).then((data: any) =>
      {
        const value :  Customer[] = [];
        this.customers     = data.vendor_info;
        this.stateName     = data.state_code
        this.paymentTerm   = data.payment_terms;
        if(data != null)
        {
            this.totalData = data.vendor_info.length;
            data.vendor_info.map((res: Customer, index: number) => {
...
public getMoreData(event: string): void {
 ...
 exportExcel() {
  ...
  TableExportUtil.exportToExcel(exportData, 'Vendor List');
 }
...
addCustomer()
  ...
  onRowClick(row : any)
  {
    ...
    this.Edit_data.controls['vendorType'].setValue(row.vendor_type)
    ...
    this.Edit_data.controls['placeOfSupply'].setValue(row.place_from_supply_code);
    this.Edit_data.controls['taxType'].setValue(row.tax_mode);
    this.Edit_data.controls['taxEmpty'].setValue(row.tax_empty);
  ...
  {
    let editproduct=this.Edit_data.get('contacts_edit') as FormArray;
        const delete_data = editproduct.at(i).value;
          if(delete_data.id !== '')
          {
            this.api.get('delete_data.php?authToken='+environment.authToken+'&table=vendor_contact&field=vendor_cont_id&id='+delete_data.id).then((data: any) =>
              {
                if (data.status == 'success')
                  {
                      if (editproduct.length > 1)
                        {
                          editproduct.removeAt(i)
                          this.toastService.typeSuccess('Deleted Successfull');
                        }
                      else
                        {
                          editproduct.reset()
 ...
 type1()
  {
    ...
    this.panhide       = true;
    this.statehide     = true;
    this.vendor_data.controls['pan'].reset();
    this.vendor_data.controls['placeOfSupply'].reset();
  }
 ...
 gstTreat(event :any)
  {
   ...
   onInputChange()
   {
    let value :any;
     ...
     value   = this.customers.find((item: { company_name: any; }) => item.company_name === billNoValue);
        ...
        if(value != undefined)
        {
         this.toastService.typeError(' Vendor Name has already been entered')
        }
        if(value == undefined)
        {
        }
   ...
   {
     let con = this.vendor_data.get('contacts') as FormArray;
     con.removeAt(i);
   }
   ...
   addPerson_wizard(prefix = "Mr.", fname = "", lname = "", contactEmail = "", workPhone = "", mobile = "")
   {
     ...
     let contacts = this.vendor_data.get('contacts') as FormArray;
...
get contacts(): FormArray {
  return this.vendor_data.get('contacts') as FormArray;
}
  ...
  copyAddress()
  {
    let address = this.vendor_data.value;
  ...
  submit(event:Event)
  {
    ...
    Object.keys(this.vendor_data.controls).forEach(field => {
      this.vendor_data.get(field)?.markAsTouched({ onlySelf: true });
    });
     ...
     value_2 = this.customers.find((item: { pan_number: any; }) => item.pan_number   === panvalue);
      ...
      if(value == undefined)
      {
        if( value_1 == undefined)
        {
          if(value_2 == undefined)
          {
       if(this.vendor_data.valid)
             {
                this.loading=true;
                 this.api.post('mp_vendor_create.php?authToken=' + environment.authToken, this.vendor_data.value ).then((data: any) => {
                    if (data.status == "success")
                    {
                      this.toastService.typeSuccess('Vendor Added Succesfully');
                     // this.ngToast.success('Customer Added Succesfully', 'Success Message', {  positionClass: 'toast-bottom-right'  });
                      ...
                      this.vendor_data.controls['Name'].setValue('');
                      this.vendor_data.controls['companyName'].setValue('');
                      this.vendor_data.controls['email'].setValue('');
                      this.vendor_data.controls['phone'].setValue('');
                      this.vendor_data.controls['mobile'].setValue('');
                      this.vendor_data.controls['website'].setValue('');
                      this.vendor_data.controls['gstTreatment'].setValue(1);
                      this.vendor_data.controls['gstin'].setValue('');
                      this.vendor_data.controls['pan'].setValue('');
                      this.vendor_data.controls['placeOfSupply'].setValue('33');
                      this.vendor_data.controls['taxType'].setValue('1');
                      this.vendor_data.controls['taxEmpty'].setValue('');
                      this.vendor_data.controls['paymentTerms'].setValue('1');
                      this.vendor_data.controls['opening_balance'].setValue(0);
                      this.vendor_data.controls['terms_condition'].setValue('');
                      this.vendor_data.controls['remarks'].setValue('');
                      this.vendor_data.controls['notes'].setValue('');
                      this.vendor_data.controls['customerStatus'].setValue(1);
                      this.vendor_data.controls['billAttention'].setValue('');
                      this.vendor_data.controls['billAddress1'].setValue('');
                      this.vendor_data.controls['billAddress2'].setValue('');
                      this.vendor_data.controls['billCity'].setValue('');
                      this.vendor_data.controls['billState'].setValue('TAMIL NADU');
                      this.vendor_data.controls['billZip'].setValue('');
                      this.vendor_data.controls['billPhone'].setValue('');
                      this.vendor_data.controls['billDefault'].setValue(1);
                      this.vendor_data.controls['billType'].setValue(1);
                      this.vendor_data.controls['billStatus'].setValue(1);
                      this.vendor_data.controls['shipAttention'].setValue('');
                      this.vendor_data.controls['shipAddress1'].setValue('');
                      this.vendor_data.controls['shipAddress2'].setValue('');
                      this.vendor_data.controls['shipCity'].setValue('');
                      this.vendor_data.controls['shipState'].setValue('');
                      this.vendor_data.controls['shipZip'].setValue('');
                      this.vendor_data.controls['shipPhone'].setValue('');
                      this.vendor_data.controls['shipType'].setValue(2);
                      this.vendor_data.controls['shipDefault'].setValue(1);
                      this.vendor_data.controls['shipStatus'].setValue(1);
                      this.vendor_data.controls['contactStatus'].setValue(1);
  ...
  edit_submit()
  {
      ...
      if (this.Edit_data.valid)
        {
         this.loading=true;
          this.api.post('mp_vendor_update.php?value='+this.vendor_id+'&authToken=' + environment.authToken, this.Edit_data.value).then((data: any) =>
         {

           if (data.status == "success")
           {
             this.loading=false;
             this.toastService.typeSuccess('Vendor Updated Succesfully');
             this.getTableData();
             this.show = true;
             this.edit_form = false;

...
public formShow          = true;
  ...
  ReturnToList()
  {
    ...
    this.router.navigate(['/purchase', 'purchase_order']);
    this.bill_addr          = null;
    this.billFrom           = null;
  ...
  onInputChange()
  {
    ...
    let value = this.VendorPOList.find((item: { po_number: any; }) => item.po_number === billNoValue);
  ...
  FinalTotalCalculation()
  {
    ...
    this.po.controls['total'].setValue((Total_Calculation).toFixed(2));
  }
    ...
    if (this.po.valid)
    {
      const billNoValue = bill_data.dcNo;
         if(this.VendorPOList != null)
          {
           var value = this.VendorPOList.find((item: { po_number: any; }) => item.po_number === billNoValue);
          }
          else{
            value = undefined;
          }

...
import { Tax_models } from './tax_models';
...
export class TaxComponent implements OnInit {
   ...
   dataSource   !: MatTableDataSource<Tax_models>;
   public totalPages   = 0;
   public Toggledata   = false;
   public      show    = false;
   public searchDataValue =''
    list          :Array<Tax_models> =[];
    from_date          : any
    to_date            : any
    tax      !: FormGroup
    edit_tax !: FormGroup

    @ViewChild("Edit_tax", { static: true }) Edit_tax!: ElementRef;
      ...
      if (data != null)
        {
             ...
             let value : Tax_models [] = []
            this.totalData    = total_data.length
            total_data.map((res: Tax_models, index: number) => {
       ...
       this.dataSource = new MatTableDataSource<Tax_models>(value);
     }
  ...
  onRowClick(row:any)
  {
      ...
      //  this.show =true
      this.edit_tax.controls['name'].setValue(row.name);
      this.edit_tax.controls['type'].setValue(row.type);
      this.edit_tax.controls['rate'].setValue(row.rate);

      const modalElement = this.Edit_tax.nativeElement;
      $(modalElement).modal('show');
       this.id = row.id
  }


  submit_tax(value:any)
  {
    if (this.edit_tax.valid)
    {
      ...
      this.api.post('post_update_data.php?table=tax&field=id&value=' + this.id + '&authToken=' + environment.authToken, this.edit_tax.value).then((data: any) => {
        ...
        {
          this.loading=false;
          this.toastService.typeSuccess('Tax Updated Succesfully');
          const modalElement = this.Edit_tax.nativeElement;
          ...
          //this.tax.reset();
              ...
              this.loading=false; }
      }).catch(error => { this.toastService.typeError('Tax Updated Failed!!');
                        this.loading=false;});
    }
    else
    {
  ...
  submit_newtax(value :any)
  {
      Object.keys(this.tax.controls).forEach(field => {
        this.tax.get(field)?.markAsTouched({ onlySelf: true });
      });
    ...
    if (this.tax.valid)
    {
      ...
      this.api.post('post_insert_data.php?table=tax&authToken=' + environment.authToken, value).then((data: any) => {
        ...
        {
          this.loading=false;
          this.toastService.typeSuccess('Tax Added Succesfully');
          this.get_tabledata()
          this.tax.reset();
          const modalElement = this.add_tax.nativeElement;
          $(modalElement).modal('hide');
        }
        else { this.toastService.typeError(data.status);
              this.loading=false; }

      }).catch(error => { this.toastService.typeError(' Tax Added Failed!!');
                        this.loading=false;});
    }
    else
    {
  ...
  {
    this.list = [];
    await this.dataSource.filteredData.map((res: Tax_models, index: number) =>
      {
        const serialNumber = index + 1;

...
@Component({
  ...
  styleUrls: ['./add-dc.component.scss'],
})
...
public formShow          = true;
...

  {
    this.dc = fb.group(
     ...
     this.SelectCategory = fb.group({
      dctype:[null, Validators.compose([Validators.required])],
     })
  ...
  ReturnToList()
  {
    ...
    this.dc.controls['tax_type'].setValue(null);
    this.dc.controls['status'].setValue(1);
    this.router.navigate(['/sales', 'delivery_challan']);
    this.bill_addr          = null;
    this.billFrom           = null;
  ...
  get product(): FormArray {
    return this.dc.get('product') as FormArray;
  }
  ...
  onChange(data:any)
  {
    ...
    this.dc.controls['customerId'].setValue(name?.customer_id);
  }
 ...
 await this.api.get('mp_dc.php?&value=' + this.customer_id + '&authToken=' + environment.authToken).then((data: any) =>
  ...
  onActivate(event:any)
  {
    if (event.type === "click")
    {
      ...
      this.api.get('mp_customer_dc_pdf.php?value=' + event.row.bill_id + '&authToken=' + environment.authToken).then((data: any) => {

        this.dcPdf            = data;
        this.img_path         = environment.baseURL + "download_file.php?path=upload/company/" + this.dcPdf[0].pdf_logo + "&authToken=" + environment.authToken
        this.dcItems          = this.dcPdf[0].dc_items;
        this.company_mobile   = this.dcPdf[0].company_details[0].mobile
        this.company_gst_no   = this.dcPdf[0].company_details[0].gst_number
        this.company_tan_no   = this.dcPdf[0].company_details[0].tan_number
        this.company_pdf_logo = this.dcPdf[0].company_details[0].pdf_logo;
  ...
  async onSubmit(bill_data :any)
  {
    ...
    Object.keys(this.dc.controls).forEach(field => {
      this.dc.get(field)?.markAsTouched({ onlySelf: true });
    });
    if (this.dc.valid)
    {
  ...
  async submit(updateAddress : any)
  {
    ...
    ReloadBillAddr(event: any)
    {
       ...
       this.api.get('get_data.php?table=customer_address&find=cust_addr_id&value=' + this.editAddressId + '&authToken=' + environment.authToken).then((data: any) => {
        this.bill_addr = data[0];
        console.log("bill_addr : ",this.bill_addr)
        this.billFrom = this.bill_addr.cust_addr_id;
        this.billAttention = this.bill_addr.attention;
        this.billAddress_line_1 = this.bill_addr.address_line_1;
        this.billAddress_line_2 = this.bill_addr.address_line_2;

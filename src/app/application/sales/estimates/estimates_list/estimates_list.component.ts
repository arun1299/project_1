...
import { invoice_detail } from './estimates_list_models';
declare let $: any;

@Component({
  selector: 'app-estimates_list',
  templateUrl: './estimates_list.component.html',
  styleUrls: ['./estimates_list.component.scss']
})
export class Estimates_listComponent implements OnInit {
 ...
 load_paymentTransactiond()
  {
    ...
    this.invoice_list   = [];
     this.api.get('mp_customer_estimate.php?&authToken=' + environment.authToken).then((data: any) =>
    {
    ...
    }).catch(error => { this.toastService.typeError('Estimate Data Load Error')});
  }
  ...
  addInvoice()
  {
    this.router.navigate(['/sales', 'estimates', 'add-estimate']);

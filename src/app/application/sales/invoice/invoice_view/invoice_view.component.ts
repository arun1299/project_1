...
import { paymentmade_list, user_account ,all_account, cash_account, company_account, gst_account} from './invoice-view-models';
...
@Component({
  selector: 'app-invoice_view',
  templateUrl: './invoice_view.component.html',
  styleUrls: ['./invoice_view.component.scss']
})
export class Invoice_viewComponent implements OnInit {
  ...
  bill_from_addr             : any;
  bill_from_To               : any;
  bill_from_Attention        : any;
  bill_from_Address_line_1   : any;
  bill_from_Address_line_2   : any;
  bill_from_City             : any;
  bill_from_State            : any;
  bill_from_Zipcode          : any;

  shipp_from_addr            : any;
  ship_from_To               : any;
  ship_from_Attention        : any;
  ship_from_Address_line_1   : any;
  ship_from_Address_line_2   : any;
  ship_from_City             : any;
  ship_from_State            : any;
  ship_from_Zipcode          : any;
  category_list              : any;
  uom_list                   : any;
  company_address            : any;
  ...
  async loadonce()
  {
    await this.api.get('mp_customer_invoice_pdf.php?value=' + this.view_invoice + '&authToken=' + environment.authToken).then((data: any) => {
              ...
              this.invoice_type   = this.invoicePdf[0].inv_type;
...
clone_address(id: string)
{
 ...
 async FetchAddress(data: any)
  {
      ...
      for (let i = 0; i < data.bill_address.length; i++)
      {
        if (data.bill_address[i] ) {
          ...
          this.billTo    = this.bill_addr.cust_addr_id;
          ...
          this.Edit_invoice.controls['billTo'].setValue(this.billTo);
        }

        if (data.ship_address[i] ) {
          this.shipp_addr = data.ship_address[i];
          this.shipTo   = this.shipp_addr.cust_addr_id;
          ...
          this.Edit_invoice.controls['shipTo'].setValue(this.shipTo);
      }
    }

    for (let j = 0; j < data.company_bill_address.length; j++)
      {

                this.bill_from_addr = data.company_bill_address[j]
                this.bill_from_To             = this.bill_from_addr.id;
                this.bill_from_Attention      = this.bill_from_addr.attention;
                this.bill_from_Address_line_1 = this.bill_from_addr.address_line1;
                this.bill_from_Address_line_2 = this.bill_from_addr.address_line2;
                this.bill_from_City           = this.bill_from_addr.city;
                this.bill_from_State          = this.bill_from_addr.state;
                this.bill_from_Zipcode        = this.bill_from_addr.pin_code;
                this.Edit_invoice.controls['billFrom'].setValue(this.bill_from_To);

       }

      for (let j = 0; j < data.company_ship_address.length; j++)
        {

                  this.shipp_from_addr          = data.company_ship_address[j];
                  this.ship_from_To             = this.shipp_from_addr.id;
                  this.ship_from_Attention      = this.shipp_from_addr.attention;
                  this.ship_from_Address_line_1 = this.shipp_from_addr.address_line1;
                  this.ship_from_Address_line_2 = this.shipp_from_addr.address_line2;
                  this.ship_from_City           = this.shipp_from_addr.city;
                  this.ship_from_State          = this.shipp_from_addr.state;
                  this.ship_from_Zipcode        = this.shipp_from_addr.pin_code;
                 this.Edit_invoice.controls['shipFrom'].setValue(this.ship_from_To);
      }
 ...
 async load_editpage()
 {
    ...
    setTimeout(() => {
                    ...
                    this.Edit_invoice.controls['branch'].setValue(this.invoice_list.branch);
                    ...
                    this.Edit_invoice.controls['inv_type'].setValue(this.invoice_list.inv_type);
                    ...
                    this.Edit_invoice.controls['pro_type'].setValue(this.invoice_list.project_type);
                    const product1 = this.Edit_invoice.get('product') as FormArray;
                    product1.clear();
                    this.invoiceitem_list.forEach((item: { invoice_item_id: any; item_list_id: any; item_description: any; tax_percent: any; amount: any; qty: any; total: any; uom: any; },j: any) => {
...
async edit_specItem(item: any,j: any)
  ...
  {
        await this.api.get('get_data.php?table=project&find=project_id&value=' + item + '&authToken=' + environment.authToken).then((data: any) => {
          ...
          let products = (<FormArray>this.Edit_invoice.controls['product']).value;
            products.forEach((product: { amount: number; taxes: number; }) => {
                  ...
                  this.editGST_Data.forEach((data: { rate: any; amount: number; }) => {
                    ...
                    let taxesAsNumber = typeof product.taxes === 'string' ? parseInt(product.taxes, 10) : product.taxes;

                    if (data.rate === taxesAsNumber)
                    {
 ...
 edit_SubTotalChange(j: any)
  ...
  item_delete()
  {
          ...
          this.api.get('delete_data.php?authToken='+environment.authToken+'&table=invoice_item&field=invoice_item_id&id='+this.item_DeleteId).then((data: any) =>
          {
                ...
                this.toastrService.typeSuccess('Invoice Item Deleted Successfull');
                const modalElement = this.delete_item.nativeElement;
          ...
          if (this.Edit_invoice.valid)
            {
              ...
              await this.api.post('mp_invoice_edit_submit.php?value='+invoice_id+'&authToken=' + environment.authToken, value).then((data: any) =>
              {
                ...
                {
                  this.loading=false;
                  this.toastrService.typeSuccess('Invoice Updated Succesfully');
                  this.Return();
 ...
 numberToWords(num: number): string {
          ...
          const tens = ['', '', 'Twenty', 'Thirty', 'Forty', 'Fifty', 'Sixty', 'Seventy', 'Eighty', 'Ninety'];
            ...
            if (num < 1000) {
              return units[Math.floor(num / 100)] + ' Hundred' + (num % 100 !== 0 ? ' ' + this.numberToWords(num % 100) : '');
            }
      ...
      return 'Invalid Number';
        ...
        if (decimalPart) {
          ...
          if (decimalPart.length === 1) {
            decimalWords += tens[Number(decimalPart)] + ' Paise ';
          } else {
            const digit1 = Number(decimalPart[0]);
            const digit2 = Number(decimalPart[1]);

            if (digit1 === 1) {
              decimalWords += teens[digit2] + ' Paise ';
            } else {
              decimalWords += tens[digit1];
              if (digit2 !== 0) {
                decimalWords += ' ' + units[digit2];
              }
              decimalWords += ' Paise ';
            }
          }
              ...
              content: [
                {
                  stack: [
                    ...
                    this.get_from_Address(files),
                    this.getBillObject_from(files),
                    this.get_to_Address(files),
                    this.getBillObject_to(files),
                  ...
                  columns: [
                    {
                      stack: [
                        ...
                        {
                          text: [
                            { text: "H O : ", bold: true,fontSize: 8 ,color: 'black',},
                            { text: test.address_1 + ", " + test.address_2 + ",", fontSize: 8, color: 'black' }
                          ],
                          fontSize: 8,
                          alignment: 'left'
                        },
                        ...
                        { text: 'UDYAM: ' + test.udyam_no + '.', fontSize: 8, bold: true, alignment: 'left' },
                        {
                          text: [
                            { text: "BRANCH  :  ", bold: true,fontSize: 8 ,color: 'black',},
                            { text: files[0].bill_from_attention +",",bold: true, fontSize: 10, color: 'black' },
                            { text: files[0].bill_from_address_line_1 + ", " + files[0].bill_from_address_line_2 + ",", fontSize: 8, color: 'black' }
                          ],
                          fontSize: 8,
                          alignment: 'left'
                        },
                        { text: files[0].bill_from_city + "-" + files[0].bill_from_zip_code + ",", fontSize: 8, color: 'black', alignment: 'left' },
...

          {
            columns: [
              [
                ...
                { text: test.vehicle_number ?'P.O ref No ':'', fontSize: 9 },
              ...
              [
                ...
                { text:test.reference_number? ": "+ test.reference_number:'',bold:true, fontSize: 9 },
...
get_from_Address(files:any) {
  ...
  return {
      table: {
          ...
          body: [
              [
                  { text: 'Bill From ',  border: [false, true, true, true], fontSize: 9, bold: true, fillColor: '#f2f2f2'},
                  { text: 'Ship From ', bold: true,  fontSize: 9, fillColor: '#f2f2f2' ,border: [false, true, false, true]},
              ],
          ],
      },
  };
        ...
        [
          {
              columns: [
                    [
                      ...
                      { text: test.bill_from_attention, bold: true, fontSize: 11 , border: [false, false, false, false]},
                      { text: test.bill_from_address_line_1, fontSize: 9 , border: [false, false, false, false]},
                      { text: test.bill_from_address_line_2+','+test.bill_state, fontSize: 9 , border: [false, false, false, false]},
                      { text:  test.bill_from_zip_code, alignment: 'left', fontSize: 9 , border: [false, false, false, false]},
          ...
          },
          {
            columns: [
              [
              { text: test.ship_from_attention, bold: true, fontSize: 11 },
              { text: test.ship_from_address_line_1, fontSize: 9 },
              { text: test.ship_from_address_line_2+','+test.bill_state, fontSize: 9 },
              { text: test.ship_from_zip_code, alignment: 'left',  fontSize: 9 },
 ...
 if(files[0].place_from_supply_code == 33)
  {
    ...
    table: {
      widths: [21, 115, 55, 30, 35, 22, 32, 22, 32, 59.5],
          body: [
            [
    ...
    return {
      table: {
        widths: [11, 120, 50, 40,40,50,60,71],
        body: [
          [
          ...
          [
            ...
            { text: '₹',fillColor: '#f2f2f2', bold: true, fontSize: 10,alignment:'center' },
        ...
        [
          {
            columns: [
              [
                ...
                { text:'Rupees '+ this.amoumt_word +' Only', fontSize: 9,bold:true, colSpan: 11, alignment: 'left',  border: [false, true, false, false]}
              ],
...

          {
            columns: [
               ...
               [
                  ...
                  { text: test.round_off != 0 ? 'Round Off':'',  fontSize: 9, colSpan: 11, alignment: 'right' ,border: [false, false, false, false]},
              ...
              [
                  ...
                  { text: test.round_off != 0 ? round_off:'' , fontSize: 9, alignment: 'right',border: [false, false, false, false] },
    ...
    table: {
      ...
      body: [
        [
          ...
          {
            stack: [
              { text: 'Authorized Signature :', bold: true, fontSize: 9, alignment: 'center' }
            ],
            margin: [0, 70, 0, 0], // Adjust the top margin to position the text at the bottom
            border: [false, true, false, false],
            alignment: 'center'
          },
        ],
    ...
    table: {
      ...
      body: [
        [
          ...
          {
            text: '', // Second cell is empty but with borders
            border: [false, false, false, false]
          }
        ]
      ],
...
getTermsObject(files:any) {
  ...
  return {
    ...
    layout: {
        ...
        layout: 'lightHorizontalLines',
        hLineWidth: function (i:any, node:any) {    return (i === 0 || i === node.table.body.length) ? 1 : 0; },
    },
  }
}
              ...
              if(this.balance != 0)
              {
                      if(last_total >= data.amount)
                      {
                        ...
                        await this.api.get('get_data.php?table=payment_transactions&authToken=' + environment.authToken).then((data: any) =>
                          ...
                          {
                            checking = data.some((item: { receipt_no: any; }) =>  normalizeString(item.receipt_no) ===  normalizeString(data.receipt_no) );
                          }).catch(error =>
                            {
                                this.toastrService.typeError('API Faild : Invoice number checking failed');
                                this.loading = false;
                            });
                          if(!checking)
                          {
                            ...
                            this.toastrService.typeError('Recipt number already exist ');
                          }
                        }
                        else
                        ...
                        this.toastrService.typeError('  Transfer is not  acceptable ,There is no balance in the invoice ');
                      }
            }
            else
            {
...
async onSubmit(bill_data: any)
            ...
            if (this.Edit_invoice.valid)
            {
              ...
              await this.api.get('get_data.php?table=invoice&authToken=' + environment.authToken).then((data: any) =>
                ...
                if(!checking)
                {
                  ...
                  this.toastrService.typeError('Invoice Number was already Exist');
                }
            }
            else
            {
 ...
 async remove()
  ...
  new_submit(value:any)
              ...
              for(let i=0 ; i< data.length; i++)
                {
                  if(data[i].type == 1)
                    {
                      this.billTo      = data[i].cust_addr_id;
                    ...
                    if(data[i].type == 2)
                      {
                        this.shipTo      = data[i].cust_addr_id;
    ...
    Reloadbill_from_Addr(event: any) {

              this.api.get('get_data.php?table=branches&find=id&value=' + event.id+ '&authToken=' + environment.authToken).then((data: any) => {
                this.bill_from_addr = data[0];

                this.bill_from_To = this.bill_from_addr.id;
                this.bill_from_Attention = 'PPV Technology praivate Ltd ('+this.bill_from_addr.branch_name+')';
                this.bill_from_Address_line_1 = this.bill_from_addr.address_line1;
                this.bill_from_Address_line_2 = this.bill_from_addr.address_line2;
                this.bill_from_City = this.bill_from_addr.city;
                this.bill_from_State = this.bill_from_addr.state;
    ...
    ReloadShipp_from_Addr(event: any) {

            this.api.get('get_data.php?table=branches&find=id&value=' + event.id+ '&authToken=' + environment.authToken).then((data: any) => {
              this.shipp_from_addr = data[0];

              this.ship_from_To = this.shipp_from_addr.id;
              this.ship_from_Attention = 'PPV Technology praivate Ltd ('+this.shipp_from_addr.branch_name+')';
              this.ship_from_Address_line_1 = this.shipp_from_addr.address_line1;
              this.ship_from_Address_line_2 = this.shipp_from_addr.address_line2;
              this.ship_from_City = this.shipp_from_addr.city;
              this.ship_from_State = this.shipp_from_addr.state;

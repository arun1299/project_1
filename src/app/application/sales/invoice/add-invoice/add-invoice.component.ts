...
public invoice_type = localStorage.getItem('invoice_type');
...
public formShow          = true;
  ...
  @ViewChild('project_type', { static: true }) project_type  !: ElementRef;
  ...
  async branch_data()
  {
    await this.api.get('get_data.php?table=branches&authToken=' + environment.authToken).then((data: any) =>
    {
...
close()
{
  ...
  ReturnToList()
  {
    ...
    this.router.navigate(['/sales', 'invoice', 'invoice-list']);
    this.bill_addr          = null;
    this.billTo           = null;
  ...
  async VendorSelection(id : any)
  {
    ...
    this.invoice.reset();
    this.invoice.controls['inv_type'].setValue(this.invoice_type);
 ...
 await this.api.get('mp_invoice.php?&value=' + this.customer_id + '&authToken=' + environment.authToken).then((data: any) =>
    ...
    if(this.invoice_type == "project")
      {
       this.api.get('mp_projectList_load.php?value=' + this.customer_id + '&authToken=' + environment.authToken).then((data: any) =>
         ...
         }).catch(error => { this.toastrService.typeError('Something went wrong in Project Load'); });
      }
  }
  ...
  {
    for (let i = 0; i < data.bill_to_address.length; i++)
    {

        if (data.bill_to_address[i].set_as_default === 1 && data.bill_to_address[i].type === 1 && data.bill_to_address[i].status === 1 ) {

          this.bill_addr          = data.bill_to_address[i];
          ...
          this.invoice.controls['billTo'].setValue(this.billTo);

        }

      if (data.ship_to_address[i].set_as_default === 1 && data.ship_to_address[i].type === 2 && data.ship_to_address[i].status === 1 ) {

        this.shipp_addr         = data.ship_to_address[i];
       ...
       this.invoice.controls['shipTo'].setValue(this.shipTo);

      }
    }

    for (let j = 0; j < data.company_address.length; j++)
      {
          if(data.company_address[j].set_as_default === 1 )
            {

                this.bill_from_addr = data.company_address[j]
                ...
                this.invoice.controls['billFrom'].setValue(this.bill_from_To);
              }

              if(data.company_address[j].set_as_default === 1 )
                {
                  this.shipp_from_addr          = data.company_address[j];
                 ...
                 this.invoice.controls['shipFrom'].setValue(this.ship_from_To);
                  }
      }
  }
  ...
  async specProject(item : any,i:any)
  {
   await this.api.get('get_data.php?table=project&find=project_id&value=' + item + '&authToken=' + environment.authToken).then((data: any) => {
        ...
        this.quantity     = 1;
        this.amount       = 0;
        this.descriptions = data[0].notes;
        this.uom          = "Nos";
   }).catch(error => { this.toastrService.typeError('Something went wrong'); });
  ...
  GSTCalculation_tax(value: any,j: any)
  {
    ...
    if (this.invoice.valid)
    {
      ...
      await this.api.get('get_data.php?table=invoice&authToken=' + environment.authToken).then((data: any) =>

        {
          ...
          {
              this.toastService.typeError('API Faild : Invoice number checking failed');
              this.loading = false;
          });
        if(!checking)
         {
            ...
            await this.api.post('mp_invoice_create.php?type=new_invoice&authToken=' + environment.authToken, bill_data).then((data: any) =>
            {
              ...
              if (data.status == "success")
              {
                ...
                this.ReturnToList();
                this.router.navigate(['/sales', 'invoice', 'invoice-list']);
               // this.router.navigate(['/sales', 'estimates', 'estimate-list'])
              }

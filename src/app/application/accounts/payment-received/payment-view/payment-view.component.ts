...
import { paymentreceived_list } from './payment-view-models';
...
export class PaymentViewComponent implements OnInit {
  ...
  [x: string]           :any;
  selected              :any[] = [];
  paymentlist           :Array<paymentreceived_list> =[];
  payment_tran_list     :Array<paymentreceived_list> =[];
  ...
  {
    await this.api.get('mp_payment_received.php?tran_id='+this.select_id+'&authToken=' + environment.authToken).then((data: any) =>
    {
      this.onload_data = data;
      this.databind(this.onload_data)

    }).catch(error => { this.toastrService.typeError('Something went wrong in Payment Received List'); });
  }
  async Loadpaymentlist()
  {
    await this.api.get('mp_payment_received.php?&authToken=' + environment.authToken).then((data: any) =>
    {

      this.paymentlist = data;

      data.map((res: paymentreceived_list, index: number) => {
...
if (selectedRow) {
  ...
  async download()
  {
     ...
     this.api.get('mp_payment_received.php?tran_id='+this.select_id+'&authToken=' + environment.authToken).then((data: any) =>
    {
      this.onload_data = data;
      this.pdfDownload(data);
    }).catch(error => { this.toastrService.typeError('Something went wrong in Payment Received'); });
  ...
  onActivate(event :any)
  {
    if(event.type=="click")
    {
      ...
      this.amount_received  = event.row.debit;
      ...
      this.invoice_no       = event.row.invoice_number;
      this.invoice_date     = event.row.tran_date;
      this.invoice_amount   = event.row.total;
      this.payment_amount   = event.row.debit;
  ...
  databind(data :any)
  {
    ...
    this.amount_received   = data[0].debit;
    ...
    this.invoice_no        = data[0].invoice_number;
    this.invoice_date      = data[0].tran_date;
    this.invoice_amount    = data[0].total;
    this.payment_amount    = data[0].debit;
      ...
      content: [
        ...
        { text: ' ' ,margin: [0, 10] },
        {
          columns: [
            ...
            [
              { text: 'Receive From  ',  bold: true ,margin: [0, 1]},
        ...
        { text: 'Payment for', style: 'subheader',margin: [0, 15] },
        {
          table: {
            ...
            body: [
              [
                { text: 'Invoice Number', fillColor: 'lightgray', border: [false, true, false, true], fontSize: 12, bold: true, alignment: 'center' },
              { text: 'Invoice Date', fillColor: 'lightgray', border: [false, true, false, true], fontSize: 12, bold: true, alignment: 'center' },
              { text: 'Invoice Amount', fillColor: 'lightgray', border: [false, true, false, true], fontSize: 12, bold: true, alignment: 'center' },
